...
impl<T> List<T> {
    ...
    pub fn push_back(&mut self, elem: T) {
        ...
        match self.tail.take() {
            Some(old_tail_rc) => {
                old_tail_rc.borrow_mut().next = Some(new_tail.clone());
                new_tail.borrow_mut().prev = Some(old_tail_rc);
    ...
    pub fn peek_front(&self) -> Option<Ref<T>> {
        self.head
            .as_ref()
            .map(|rc_ref| Ref::map(rc_ref.borrow(), |node| &node.elem))
    }
    ...
    pub fn peek_back_mut(&mut self) -> Option<RefMut<T>> {
        self.tail
            .as_ref()
            .map(|rc_ref| RefMut::map(rc_ref.borrow_mut(), |node| &mut node.elem))
    }
